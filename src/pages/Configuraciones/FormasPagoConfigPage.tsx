...
import type { FormaPagoConfig } from "@/types/common";
...
const FormasPagoConfigPage = () => {
  ...
  const [formasPago, setFormasPago] = useState<FormaPagoConfig[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [editItem, setEditItem] = useState<FormaPagoConfig | null>(null);
  ...
  const loadFormas = useCallback(() => {
    ...
    const fps = Array.isArray(config.formasPago) ? undefined : config.formasPago?.Venta;
  ...
  const saveFormas = (nuevas: FormaPagoConfig[]) => {
    const config = orgService.getConfig(orgId);
    config.formasPago = { ...(config.formasPago as Record<string, FormaPagoConfig[]> || {}), Venta: nuevas };
  ...
  const openEdit = (fp: FormaPagoConfig) => {
    setEditItem(fp);
  ...
  const toggleProp = (id: number, prop: keyof FormaPagoConfig, value: boolean) => {
...
interface CardFormaPagoProps {
  data: FormaPagoConfig;
  onEdit: () => void;
  onToggle: (prop: keyof FormaPagoConfig, value: boolean) => void;
}

const CardFormaPago = ({ data, onEdit, onToggle }: CardFormaPagoProps) => (
...
const MiniSwitch = ({ label, value, onChange }: MiniSwitchProps) => (
...
const ToggleRow = ({ label, value, onChange, desc }: ToggleRowProps) => (

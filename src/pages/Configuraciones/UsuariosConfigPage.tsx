...
const UsuariosConfigPage = () => {
  ...
  const [formSucursales, setFormSucursales] = useState<number[]>([]);
  ...
  const handleSave = async () => {
    ...
    const sucursalesSel: UsuarioSucursal[] = sucursales.filter((s) => formSucursales.includes(s.id)).map((s) => ({ id: s.id, nombre: s.nombre, organizacionId: s.organizacionId }));
    ...
    if (editItem) {
      ...
      result = await usuarioService.create({ ...payload, password: formPassword.trim() } as { username: string; password: string; nombre: string; sucursales?: UsuarioSucursal[] });
    }
    if (result === null) {

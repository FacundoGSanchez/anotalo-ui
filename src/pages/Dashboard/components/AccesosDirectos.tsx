import { Card, Typography } from "antd";
import { OPCIONES_TIPO, POS_COLORS } from "@/constants/posConstants";
import type { ReactNode } from "react";

const { Text } = Typography;

interface AccesoBtnProps {
  icon: ReactNode;
  label: string;
  color: string;
  onClick: () => void;
}

const AccesoBtn = ({ icon, label, color, onClick }: AccesoBtnProps) => (
  <div
    onClick={onClick}
    style={{
      textAlign: "center",
      cursor: "pointer",
      maxWidth: "56px",
      flex: "0 0 auto",
    }}
  >
    <div
      style={{
        fontSize: "26px",
        color: color,
        background: `${color}12`,
        maxWidth: "56px",
        height: "56px",
        borderRadius: "18px",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        margin: "0 auto 8px auto",
        boxShadow: `0 4px 10px ${color}15`,
      }}
    >
      {icon}
    </div>
    <Text
      strong
      style={{
        fontSize: "11px",
        display: "block",
        color: "#595959",
        wordBreak: "break-word",
        overflowWrap: "break-word",
      }}
    >
      {label.toUpperCase()}
    </Text>
  </div>
);

interface AccesosDirectosProps {
  onSelectTipo: (key: string) => void;
}

interface TipoOption {
  key: string;
  icon: ReactNode;
  [rest: string]: unknown;
}

const AccesosDirectos = ({ onSelectTipo }: AccesosDirectosProps) => {
  return (
    <Card
      title={<Text strong>Accesos Rápidos</Text>}
      style={{
        borderRadius: "20px",
        marginBottom: "16px",
        boxShadow: "0 4px 15px rgba(0,0,0,0.04)",
      }}
      styles={{ body: { padding: "12px 10px" } }}
    >
      <div
        style={{
          display: "flex",
          flexWrap: "wrap",
          gap: "6px",
          paddingBottom: "8px",
        }}
      >
        {(OPCIONES_TIPO as TipoOption[]).map((opt) => (
          <AccesoBtn
            key={opt.key}
            icon={opt.icon}
            label={opt.key}
            color={POS_COLORS[opt.key]}
            onClick={() => onSelectTipo(opt.key)}
          />
        ))}
      </div>
    </Card>
  );
};

export default AccesosDirectos;

...
interface EntidadesListadoProps {
  tipo?: string;
  onBack?: () => void;
  simple?: boolean;
}

const calcularSaldoEntidad = (entidadId: number): number => {
...
const verificarPlazoVencido = (entidadId: number, plazoDias?: number): boolean => {
...
const EntidadesListado = ({ tipo: tipoProp, onBack, simple }: EntidadesListadoProps) => {
  const { tipo: tipoParam } = useParams<{ tipo?: string }>();
  const tipo = tipoProp || tipoParam || "clientes";
      ...
      <div style={{ background: "#fff", borderRadius: "16px", border: "1px solid #f0f0f0", overflow: "hidden" }}>
        {datosFiltrados.length === 0 ? (
          ...
          datosFiltrados.map((record, idx) => {
            const ctaConfig = (record.ctaCteConfig || {}) as CtaCteConfig;
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Text type="secondary" style={{ fontSize: "12px" }}>
                      Código #{record.nro}
                      {record.telefono ? ` • Tel: ${String(record.telefono)}` : ""}
                    </Text>

...
import type { Movimiento, Rubro, LineItem } from "@/types";

const { Text } = Typography;

const RUBROS_MOCK: Rubro[] = [
...
interface ModalDetalleMovimientoProps {
  ...
  onClose: () => void;
  onUpdateList: (movimientos: Movimiento[]) => void;
}
...
const ModalDetalleMovimiento = ({
  ...
  const handleUpdateEntidad = (nuevaEntidad: { id: number; nombre: string }) => {
    const result = movimientoService.update(movimiento.id, {
      entidad: { ...nuevaEntidad, activo: true },
    });
    if (result.success) {
      onUpdateList(movimientoService.getAll());
  ...
  const cambiarRubroItem = (itemId: number, nuevoRubro: Rubro) => {
    const lineItems: LineItem[] = (movimiento.lineItems || []).map((item) =>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
              {RUBROS_MOCK.filter((r) => r.grupo === grupo).map((rubro) => {
                ...
                return (
                  <button
                    key={rubro.sigla}
                    onClick={() => editItemId != null && cambiarRubroItem(editItemId, rubro)}
                    style={{
                      padding: "10px 16px",
                      borderRadius: "12px",
                      fontSize: "13px",
    ...
    <>
      ...
      >
        <div style={{ padding: "0" }}>
          ...
          {movimiento.lineItems && movimiento.lineItems.length > 0 && (
            <div
              style={{
                border: "1px solid #f0f0f0",
                borderRadius: "16px",
                ...
                {(showAllItems
                  ...
                  <div
                    ...
                    style={{
                      ...
                      padding: "6px 10px",
                      borderBottom:
                        idx < (movimiento.lineItems?.length ?? 0) - 1
                          ? "1px solid #f0f0f0"
                          : "none",
                    }}
                  >
                    ...
                    <Button
                      ...
                      size="small"
                      icon={<MdEdit size={12} />}
                      onClick={() => { setEditItemId(item.id); setRubroModalOpen(true); }}
                      style={{
                        width: "24px",
                        height: "24px",
                        display: "flex",

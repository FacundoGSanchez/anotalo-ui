...
import type { Cierre, Movimiento } from "@/types";

const { Text } = Typography;

interface CajaEntry {
  type: "cierre" | "mov";
  data: Cierre | Movimiento;
  balance: number;
}
...
const AdminCajaPage = () => {
  ...
  const movsCaja = useMemo(() => {
    ...
    return movimientoService
      .getAll()
      .filter((m) => {
        if (m.formaPagos && m.formaPagos.length > 0) {
  ...
  const handleRegistrarMov = () => {
    ...
    const movimientoData = {
      tipo: movTipoModal as "Ingreso" | "Retiro",
      importe: Number(movImporte),
      formaPago: "Efectivo",
      formaPagos: [{ nombre: "Efectivo", importe: Number(movImporte) }],
      entidad: { id: 0, nombre: "Caja Interna", activo: true },
  ...
  const grouped = useMemo(() => {
    const groups: Record<string, CajaEntry[]> = {};
    entries.forEach((entry) => {
      const date =
        entry.type === "mov"
          ? (() => { const { fecha } = movimientoService.extraerFechaHora((entry.data as Movimiento).fechaRegistro); return fecha; })()
          : (entry.data as Cierre).fecha || "Sin fecha";
            ...
            <div key={fecha} style={{ marginBottom: "20px" }}>
              ...
              >
                {grouped[fecha].map((entry, i) => {
                  ...
                  const mov = entry.data as Movimiento;
                  const esEntrada =
                    mov.tipo === MOVIMIENTO_TIPOS.VENTA ||
                    mov.tipo === MOVIMIENTO_TIPOS.INGRESO ||
                    mov.tipo === MOVIMIENTO_TIPOS.COBRO;

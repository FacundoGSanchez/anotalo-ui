...
import type { Entity, Movimiento, FormaPagoDef } from "@/types";
...
type ModalStep = "amount" | "formaPago";

interface EditFpModalData {
  mov: Movimiento;
  fpIndex: number;
}
...
const DetalleCtaCtePage = () => {
  ...
  const [movTipoModal, setMovTipoModal] = useState<string | null>(null);
  ...
  const [editFpModal, setEditFpModal] = useState<EditFpModalData | null>(null);
  ...
  const entidad = useMemo(
    () => entidadService.getById("clientes", Number(id)),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [id, refreshKey],
  );

  const ctaCteConfig: CtaCteConfig = (entidad?.ctaCteConfig || {}) as CtaCteConfig;
  ...
  const handleRegistrarConForma = (formaPago: string) => {
    if (movImporte <= 0) {
    ...
    const movimientoData = {
      tipo: movTipoModal as "Cobro" | "Pago",
      importe: Number(movImporte),
      formaPago,
      formaPagos: [{ nombre: formaPago, importe: Number(movImporte) }],
      entidad: { id: entidad.id, nombre: entidad.nombre, activo: true },
    };
  ...
  const abrirModal = (tipoMov: string) => {
  ...
  const handleChangeFormaPago = (nuevoNombre: string) => {
  ...
  const esPositivo = (mov: Movimiento) =>
              ...
              <div key={mov.id}>
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {mov.formaPagos && mov.formaPagos.length > 1 && (
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",

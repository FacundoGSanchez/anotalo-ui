...
const fmt = (v: number): string => `${Number(v).toLocaleString("es-AR")}`;
...
interface CalcMultipleFormaPagoProps {
  open: boolean;
  formaLabel: string;
  formaColor: string;
  initialValue?: number;
...
const CalcMultipleFormaPago = ({ open, formaLabel, formaColor, initialValue = 0, importe = 0, saldo = 0, onConfirm, onCancel }: CalcMultipleFormaPagoProps) => {
        ...
        >
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <Text
              ...
              onMouseEnter={(e) => { if (saldo > 0) (e.currentTarget as HTMLElement).style.opacity = "0.7"; }}
              onMouseLeave={(e) => { if (saldo > 0) (e.currentTarget as HTMLElement).style.opacity = "1"; }}
            >
              {saldo === 0 ? "0" : fmt(saldo)}
            </Text>
          </div>

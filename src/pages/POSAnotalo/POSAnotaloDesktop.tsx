...
import type { Movimiento, LineItem } from "@/types";
...
const SidebarRow = ({ icon, label, value, color }: SidebarRowProps) => (
...
const POSAnotaloDesktop = () => {
  ...
  const [recentMovements, setRecentMovements] = useState<Movimiento[]>([]);
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        >
                          {(m.formaPagos ? m.formaPagos.map(fp => fp.nombre).join(" + ") : m.formaPago)} · {(() => { const { fecha } = movimientoService.extraerFechaHora(m.fechaRegistro); return fecha; })()}
                        </Text>
                      </div>
                    </div>

...
BEGIN
    ...
    DECLARE cur CURSOR FOR
        SELECT Id, OrganizacionId, Tipo, Nombre, Sigla, RequiereEntidad, ImpactaCaja, ImpactaCtaCte, Activo
        FROM FormaPago
    ...
    read_loop: LOOP
        FETCH cur INTO v_id, v_organizacionId, v_tipo, v_nombre, v_sigla,
                       v_requiereEntidad, v_impactaCaja, v_impactaCtaCte, v_activo;
        IF v_done THEN
            LEAVE read_loop;
        END IF;
...
BEGIN
    ...
    DECLARE cur CURSOR FOR
        SELECT Id, OrganizacionId, Sigla, Nombre, Activo
        FROM Rubros
        WHERE (v_filtroId IS NULL OR Id = v_filtroId)
          AND (v_filtroOrganizacionId IS NULL OR OrganizacionId = v_filtroOrganizacionId)
        ORDER BY Nombre;
    ...
    OPEN cur;
    read_loop: LOOP
        FETCH cur INTO v_id, v_organizacionId, v_sigla, v_nombre, v_activo;
        IF v_done THEN
            LEAVE read_loop;
        END IF;
...
BEGIN
    ...
    DECLARE cur CURSOR FOR
        SELECT Id, OrganizacionId, UsaRubro
        FROM ConfigPOS
        WHERE (v_filtroId IS NULL OR Id = v_filtroId)
          AND (v_filtroOrganizacionId IS NULL OR OrganizacionId = v_filtroOrganizacionId);
    ...
    OPEN cur;
    read_loop: LOOP
        FETCH cur INTO v_id, v_organizacionId, v_usaRubro;
        IF v_done THEN
            LEAVE read_loop;
        END IF;

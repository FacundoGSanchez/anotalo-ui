DELIMITER $$
CREATE DEFINER=`anotalo_user`@`%` PROCEDURE `SpEntidades_Eliminar`(
...
BEGIN
    ...
    UPDATE Entidades SET Activo = 0 WHERE Id = v_id AND OrganizacionId = v_organizacionId;

    SET p_result = 1;
    SET p_mensaje = 'Entidad desactivada con éxito';
